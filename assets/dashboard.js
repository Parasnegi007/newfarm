window.addEventListener("DOMContentLoaded", async function () {
    ...
    sendOtpBtn.addEventListener("click", async function () {
        ...
        try {
            ...
            const response = await fetch("http://localhost:5000/api/users/send-otp-update", {
                method: "POST",
                headers: { 
...
async function fetchAddresses() {
    const token = localStorage.getItem("authToken");
    const endpoint = "http://localhost:5000/api/users/getAddresses";
...
async function deleteAddress(addressId) {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/users/deleteAddress/${addressId}`, {
            method: "DELETE",
            headers: {
                "Authorization": `Bearer ${token}`
            }
...
async function fetchOrders() {
    const token = localStorage.getItem("authToken");
    const endpoint = `http://localhost:5000/api/orders/my-orders`;
                ...
                <div class="order-items-list">
                    ${order.orderItems.map(item => `
                        <div class="order-item">
                            ...
                            <img src="${item.productId?.image || 'fallback.jpg'}" alt="${item.name}" />
                        </div>
                    `).join('')}
                </div>
